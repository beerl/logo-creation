...
def process_logo(input_path, output_path, top_margin=73, right_margin=73, scale_factor=1.0, invert=False, horizontal_offset=0, vertical_offset=0):
    """
    ...
    try:
        ...
        if is_svg:
            # Pour les SVG, on informe l'utilisateur que le format n'est pas supporté
            raise ValueError("Le format SVG n'est pas supporté. Veuillez convertir votre logo en PNG ou JPG avant de l'utiliser.")
        ...
        # Redimensionnement proportionnel selon les spécifications exactes
        original_width, original_height = processed_img.size
        max_width, max_height = 613, 283  # Largeur max: 613px, Hauteur max: 283px
        
        width_ratio = max_width / original_width
        height_ratio = max_height / original_height
        
        # Utiliser le ratio le plus petit pour s'assurer que l'image tient dans les limites
        # sans déformation (aspect ratio préservé)
...
def process_text_logo(text, output_path, top_margin=73, right_margin=73, scale_factor=1.0, horizontal_offset=0, vertical_offset=0):
    """
    ...
    try:
        ...
        canvas_width, canvas_height = 2024, 1276
        canvas = Image.new('RGB', (canvas_width, canvas_height), (255, 255, 255))
        paste_x = canvas_width - text_img.width - right_margin + 60 + int(horizontal_offset)
        paste_y = top_margin + int(vertical_offset)
...
def process_card_logo(logo_path, output_path, card_template_path='static/card_template.png', top_margin=35, right_margin=35, max_width=210, max_height=100, scale_factor=1.0, horizontal_offset=0, vertical_offset=0):
    """
    ...
    try:
        ...
        if is_svg:
            raise ValueError("Le format SVG n'est pas supporté pour la carte.")
